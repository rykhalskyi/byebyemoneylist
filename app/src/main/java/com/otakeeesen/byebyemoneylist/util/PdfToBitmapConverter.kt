...
object PdfToBitmapConverter {
    ...
    sealed class ConversionResult {
        data class Success(val bitmap: Bitmap) : ConversionResult()
     ...
     * @return A ConversionResult containing the Bitmap or an error message.
     */
    fun convertPdfToBitmap(context: Context, uri: Uri): ConversionResult {
        return try {
            context.contentResolver.openFileDescriptor(uri, "r")?.use { pfd ->
                PdfRenderer(pfd).use { renderer ->
                    if (renderer.pageCount == 0) return ConversionResult.Error("PDF is empty")
                    ...
                    try {
                        ...
                        if (pageBitmaps.isEmpty()) return ConversionResult.Error("Failed to render pages")
                        if (pageBitmaps.size == 1) return ConversionResult.Success(pageBitmaps[0])
                        ...
                        if (totalPixels > MAX_TOTAL_PIXELS) {
                            ...
                            resultBitmap = Bitmap.createBitmap(newMaxWidth, newTotalHeight, Bitmap.Config.ARGB_8888)
                            ...
                            resultBitmap = Bitmap.createBitmap(maxWidth, totalHeight, Bitmap.Config.ARGB_8888)
                            val canvas = Canvas(resultBitmap)
                            canvas.drawColor(Color.WHITE)
                        ...
                        return ConversionResult.Success(resultBitmap)
                    } catch (e: Exception) {
            ...
            } ?: ConversionResult.Error("Could not open file")
        } catch (e: Exception) {
            Log.e(TAG, "Failed to convert PDF to Bitmap", e)
            ConversionResult.Error("Error: ${e.localizedMessage ?: "Unknown error"}")
        }
    }
}
