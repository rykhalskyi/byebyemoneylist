# Wiki Index

## Overview
- [project-overview](pages/project-overview.md) — Tech stack, architecture, folder structure, key concepts. Created 2026-08-08.
- [build-deploy](pages/build-deploy.md) — Build, test, lint, and deploy instructions. Created 2026-08-08.

## Specs
- [specs/01-dashboard-ux-improvements](pages/specs/01-dashboard-ux-improvements.md) — Category emojis, empty list with category, streamlined Quick Purchase. Created 2026-08-09.
- [specs/02-nextcloud-sync-redesign](pages/specs/02-nextcloud-sync-redesign.md) — Complete Nextcloud sync: settings redesign + Store and Product sync. Created 2026-09-03.
- [specs/03-placeholder-list-items](pages/specs/03-placeholder-list-items.md) — New-list placeholders: free-text items, scan-a-paper-list, LLM-first purchase reconciliation, "not bought" state, analog marking. Created 2026-09-22.

## Plans
- [plans/01-dashboard-ux-improvements](pages/plans/01-dashboard-ux-improvements.md) — Implementation plan for Dashboard UX epic. Created 2026-08-09.
- [plans/02-category-emojis](pages/plans/02-category-emojis.md) — Issue #50: category emoji icons (focused plan, Task 1 of epic). Created 2026-08-10.
- [plans/03-quick-purchase-flow](pages/plans/03-quick-purchase-flow.md) — Issue #52: streamlined Quick Purchase flow (focused plan, Task 3 of epic). Created 2026-08-11.
- [plans/04-widget-reordering](pages/plans/04-widget-reordering.md) — Drag-and-drop dashboard widget reordering. Created 2026-08-12.
- [plans/05-scan-purchase-widget](pages/plans/05-scan-purchase-widget.md) — Issue #54: Scan Purchase widget (LLM-gated shortcut to PurchaseDialog). Created 2026-08-13.
- [plans/06-nextcloud-sync-redesign](pages/plans/06-nextcloud-sync-redesign.md) — Implementation plan for Nextcloud sync epic (3 tickets: categories refactor, stores, products). Created 2026-09-03.
- [plans/07-sync-delta-git-model](pages/plans/07-sync-delta-git-model.md) — Phase 2 (tickets 4–6): git-like delta sync — change detection, update propagation, conflict resolution. Created 2026-09-04.
- [plans/08-shopping-list-sync](pages/plans/08-shopping-list-sync.md) — Phase 3: shopping list mirror sync (no match routine, serverId-linked, bidirectional creation). Created 2026-09-05.
- [plans/09-sync-enhancements](pages/plans/09-sync-enhancements.md) — Follow-ups to tickets 4–6: pending-update hints on the sync settings hub, syncing store category+address (server+client), git-style shopping-list sync + ShoppingListSyncScreen. Created 2026-09-06.
- [plans/10-placeholder-list-items](pages/plans/10-placeholder-list-items.md) — Ticket 1 of the placeholder epic: model + migration 29→30, manual add/display, placeholder editing, sync deferral guard. Created 2026-09-22.
- [plans/11-placeholder-reconciliation](pages/plans/11-placeholder-reconciliation.md) — Ticket 2: silent automatic reconciliation of placeholders against a purchase (LLM-first + fallback), not-bought state, per-item re-link. Created 2026-09-22.

## Tickets
- [tickets/01-issue-52-quick-purchase](pages/tickets/01-issue-52-quick-purchase.md) — Issue #52: Quick Purchase flow ticket. Created 2026-08-11.
- [tickets/02-issue-54-scan-purchase](pages/tickets/02-issue-54-scan-purchase.md) — Issue #54: Scan Purchase widget ticket. Created 2026-08-13.

## Research
- [research/01-dashboard-epic-analysis](pages/research/01-dashboard-epic-analysis.md) — Full analysis of Dashboard UX epic requirements vs current codebase. Created 2026-08-09.
- [research/02-month-sum-calculation](pages/research/02-month-sum-calculation.md) — Canonical month spending sum: the single formula + centralized `sumExpenses` used across widget, list screen, and analytics. Created 2026-08-14.
- [research/03-nextcloud-sync-redesign](pages/research/03-nextcloud-sync-redesign.md) — Nextcloud sync research: client + server API state, gaps, ordering and matching strategy. Created 2026-09-03.
- [research/04-sync-delta-git-model](pages/research/04-sync-delta-git-model.md) — Git-inspired delta sync: content-hash change detection, two-phase reconciliation/delta, asymmetric field handling. Created 2026-09-04.